const APP_ID = "9f2ba29f4a3649e9a4025e7f7b9457dc";
...
let initAgoraRTM_Client = async () => {
    rtmClient = await AgoraRTM.createInstance(APP_ID)
    await rtmClient.login({uid,token})
    await rtmClient.addOrUpdateLocalUserAttributes({'name':displayName})    
}

let initAgoraRTM_Channel = async () => {
    ...
    getMembers()
    addBotMessageToDom(`${displayName} has joined the chat.`)
}


let initAgoraRTC_Client = async () => {
...
let setupMeetingRoom = async () => {

    await initAgoraRTM_Client();
    await initAgoraRTM_Channel();
    await initAgoraRTC_Client();
